...
// konfigurasi scene di-inject oleh Blade (viewer.blade.php) sebagai window.sceneConfig
const loadSceneEntry = async () => {
    const entry = window.sceneConfig;
    if (!entry?.src) throw new Error('Konfigurasi scene tidak ditemukan');
    return entry;
};
...
const main = async () => {
    ...
    // judul sudah dirender server (Blade); baris ini hanya fallback
    const titleEl = document.getElementById('scene-title');
    if (titleEl) titleEl.textContent = entry.title ?? entry.id;
